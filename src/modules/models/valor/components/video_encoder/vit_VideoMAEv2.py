...
class VisionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 return_feat_map_foreach_layer: bool = False, #! Added by me to allow extracting features at each layer
                 **kwargs) -> None:
        ...
        self.return_feat_map_foreach_layer = return_feat_map_foreach_layer #! Added by me to allow extracting features at each layer
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.return_feat_map_foreach_layer: #! Added by me to allow extracting features at each layer
            feat_map_each_layer = []    
        
        for blk in self.blocks:
            x = blk(x)            
            if self.return_feat_map_foreach_layer:
                feat_map_each_layer.append(x.reshape(b, -1, h, w, self.embed_dims)) #! Added by me to allow extracting features at each layer

        if self.return_feat_map_foreach_layer: #! Added by me to allow extracting features at each layer
            return feat_map_each_layer
